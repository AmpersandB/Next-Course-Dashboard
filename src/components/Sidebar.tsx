import Image from 'next/image';
import { IoBrowsersOutline, IoCalculator, IoFootball, IoHeart, IoLogoReact } from 'react-icons/io5';
import { SidebarMenuItem } from '.';

const menuItems = [
    {
        path: '/dashboard/main',
        icon: <IoBrowsersOutline size={40} className='mr-2'/>,
        title: 'Dashboard',
        subtitle: 'Main Page'
    },
    {
        path: '/dashboard/counter',
        icon: <IoCalculator size={40} className='mr-2'/>,
        title: 'Counter',
        subtitle: 'Shopping cart counter'
    },
    {
        path: '/dashboard/pokemons',
        icon: <IoFootball size={40} className='mr-2'/>,
        title: 'Pokemons',
        subtitle: 'Static generated'
    },
    {
        path: '/dashboard/favorites',
        icon: <IoHeart size={40} className='mr-2'/>,
        title: 'Favorites',
        subtitle: 'Global State'
    },
    
    ]


export const Sidebar = () => {
  return (
    <div 
    id="menu" 
    className="bg-gray-900 min-h-screen z-10 text-slate-300 w-64 left-0 overflow-y-scroll"
    style={{width:'400px'}}
    >
    <div id="logo" className="my-4 px-6">
      <h1 className="flex items-center text-lg md:text-2xl font-bold text-white">
        <IoLogoReact className='mr-2'/>
        Dash
        <span className="text-blue-500">board</span>
        .
      </h1>
      <p className="text-slate-500 text-sm">NextJS Course Project</p>
    </div>

    <div id="profile" className="px-6 py-10">
      <p className="text-slate-500">Welcome back,</p>
      <a href="#" className="inline-flex space-x-2 items-center">
        <span>
          <Image 
          className="rounded-full w-8 h-8" 
          src="https://res.cloudinary.com/dpcluaxfc/image/upload/v1702873495/profile1_yxoprp.jpg" 
          alt="Image" 
          width={50}
          height={50}
          />
        </span>
        <span className="text-sm md:text-base font-bold">
          Luis H. Pinillo
        </span>
      </a>
    </div>
    <div id="nav" className="w-full px-6">
        {
        menuItems.map((item) => (
            <SidebarMenuItem key={item.path} {...item} />
        ))
        }
     

    </div>
  </div>
  )
}
